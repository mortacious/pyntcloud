...
from .plot import DESCRIPTION#, plot_PointCloud
...
class PointCloud(object):
    ...
    def __init__(self,
        ...
        kwargs: custom metadata attributes
        """
        self.points = points
        #self.mesh = mesh
        ...
        self.vertices = self.points[["x", "y", "z"]].values

        # store normals if available
        self.normal_fields = normal_fields
        self.normals = None
        if set(normal_fields).issubset(self.points.columns):
            self.normals = self.points[self.normal_fields].values
    ...
    # @mesh.setter
    # def mesh(self, df):
    #     # allow PointCloud to don't have mesh assigned
    #     if df is not None:
    #         if not isinstance(df, pd.DataFrame):
    #             raise TypeError("Mesh argument must be a DataFrame")
    #         elif not {'v1', 'v2', 'v3'}.issubset(df.columns):
    #             print(df.columns)
    #             raise ValueError(
    ...
    def from_file(cls, filename, **kwargs):
        ...
        -------
        PointCloud: object
            PointCloud instance, containing all valid elements in the file.
        """
        ...
        if ext not in FROM:
            ...
            return cls(**FROM[ext](path, **kwargs))

    def to_file(self, filename, also_save=None, **kwargs):
        """Save PointCloud data to file.
        ...
        also_save: list of str, optional
            Default: None
            Names of the attributes that will be extracted from the PointCloud
        ...
        kwargs["filename"] = os.path.expanduser(filename)
    ...
    def add_field(self, name, values):
        """ Add one custom column to PointCloud.points
        ...
        if values.shape[0] != self.points.shape[0]:
            raise ValueError("Not enough values for this point cloud of length", self.points.shape[0])
        self.points[name] = values

        return name

    def add_scalar_field(self, name, **kwargs):
        """Add one or multiple columns to PointCloud.points.
        ...
        **ONLY REQUIRE XYZ**
        ...
        # if the scalar field is predefined
        if name in ALL_SF:
            scalar_field = ALL_SF[name](pointCloud=self, **kwargs)
    ...
    #     """Build a structure and add it to the corresponding PointCloud's attribute.
    #
    #     Parameters
    #     ----------
    #     name: str
    ...
    #     """Compute filter over PointCloud's points and return it.
    #
    #     Parameters
    #     ----------
    #     name: str
    ...
    #         pointcloud_filter = ALL_FILTERS[name](PointCloud=self, **kwargs)
    ...
    # def get_sample(self, name, as_PointCloud=False, **kwargs):
    ...
    #         See below.
    #
    #     as_PointCloud: bool, optional
    #         Default: False
    #         If True, sampled points will be returned as PointCloud instance.
    #
    #     kwargs
    #         Vary for each name.
    #         See below.
    ...
    #     Returns
    #     -------
    #     sampled_points: (n, 3) ndarray or PointCloud
    #         'n' vary for each method. PointCloud if as_PointCloud
    #
    #     Notes
    #     -----
    #
    ...
    #         sampler = ALL_SAMPLERS[name](PointCloud=self, **kwargs)
    ...
    #         - If **kdtree** is None:
    #
    #         The KDTree will be computed and added to PointCloud as part of the process.
    #
    #         - Else:
    #
    ...
    #     """Divide the PointCloud using unique values in given sf.
    #
    #     This function will generate PointClouds by grouping points using the unique
    ...
    #         Default: "ply"
    #         Extension used to save the generated PointClouds.
    #         Must be of one of the formats present in pycloud.io.TO
    #
    #     save_path: str, optional
    #         Default: "."
    #         Path where the PointClouds will be saved.
    #     """
    #     scalar_field = self.points[scalar_field]
    #
    #     splits = {x: PointCloud(self.points.loc[scalar_field == x]) for x in scalar_field.unique()}
    #
    ...
    def _update_points(self, df):
        ...
        self.structures = StructuresDict()
        self.__points = df
        self.vertices = self.__points[["x", "y", "z"]].values
        self.centroid = self.vertices.mean(0)
    ...
    #     cmap="hsv",
    #     output_name="PointCloud_plot",
    #     IFrame_shape=(800, 500),
    #     polylines={}
    #     ):
    #     """Visualize PointCloud in a Jupyter notebook using three.js.
    #
    #     Parameters
    #     ----------
    ...
    #         colors = np.repeat([[255, 125, 0]], self.vertices.shape[0], axis=0)
    #
    #     colors = colors.astype(np.uint8)
    #
    #     points = pd.DataFrame(self.vertices, columns=["x", "y", "z"])
    #
    #     for n, i in enumerate(["red", "green", "blue"]):
    #         points[i] = colors[:, n]
    #
    #     new_PointCloud = PointCloud(points)
    #
    #     if mesh and self.mesh is not None:
    #         new_PointCloud.mesh = self.mesh[["v1", "v2", "v3"]]
    #
    #     return plot_PointCloud(
    #         new_PointCloud,
