...
from pycloud.scalar_fields.k_neighbors import (
    EigenValues,
    EigenDecomposition,
    UnorientedNormals
)
...
@pytest.mark.parametrize("ScalarField", [
    EigenValues,
    EigenDecomposition,
    UnorientedNormals
])
...
def test_EigenValues_bounds(pyntcloud_with_rgb_and_normals, pyntcloud_with_rgb_and_normals_k_neighbors):
    scalar_field = EigenValues(
...
@pytest.mark.usefixtures("plane_pyntcloud", "plane_k_neighbors")
def test_EigenDecomposition_coplanar_points_some_fields_are_0(plane_pyntcloud, plane_k_neighbors):
...
@pytest.mark.usefixtures("pyntcloud_with_rgb_and_normals", "pyntcloud_with_rgb_and_normals_k_neighbors")
def test_EigenDecomposition_bounds(pyntcloud_with_rgb_and_normals, pyntcloud_with_rgb_and_normals_k_neighbors):
    scalar_field = EigenDecomposition(
        pyntcloud=pyntcloud_with_rgb_and_normals,
        k_neighbors=pyntcloud_with_rgb_and_normals_k_neighbors)
    scalar_field.extract_info()
    ...
    for x in [
        "e1(4)", "e2(4)", "e3(4)",
        "ev1_x(4)", "ev1_x(4)", "ev1_x(4)",
        "ev2_x(4)", "ev2_y(4)", "ev2_y(4)",
        "ev3_x(4)", "ev3_z(4)", "ev3_z(4)"
    ]:
        assert all(scalar_field.to_be_added[x] >= -1)
        assert all(scalar_field.to_be_added[x] <= 1)
...
@pytest.mark.usefixtures("plane_pyntcloud", "plane_k_neighbors")
def test_UnorientedNormals_coplanar_points_nx_ny_are_0(plane_pyntcloud, plane_k_neighbors):
...
@pytest.mark.usefixtures("pyntcloud_with_rgb_and_normals", "pyntcloud_with_rgb_and_normals_k_neighbors")
def test_UnorientedNormals_bounds(pyntcloud_with_rgb_and_normals, pyntcloud_with_rgb_and_normals_k_neighbors):
    scalar_field = UnorientedNormals(
        pyntcloud=pyntcloud_with_rgb_and_normals,
        k_neighbors=pyntcloud_with_rgb_and_normals_k_neighbors)
    scalar_field.extract_info()
